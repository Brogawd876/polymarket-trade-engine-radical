...
describe("Pair Validator", () => {
  ...
  test("malformed raw L2 NDJSON fails", async () => {
    ...
    fs.writeFileSync(replayLog, JSON.stringify({ type: "slot", action: "start", startTime: 1000, endTime: 5000 }) + "\n" + JSON.stringify({ type: "market_price", kind: "open", openPrice: 60000, ts: 1500 }) + "\n" + JSON.stringify({ type: "chainlink_resolution", kind: "close", price: 60000, roundId: "123", chainUpdatedAtMs: 4000, ts: 4500 }) + "\n" + JSON.stringify({ ts: 1000 }) + "\n");
  ...
  test("complete coverage works with out-of-order lines", async () => {
    ...
    expect(manifest.coverageLeadMs).toBe(1500 - 1000); // 500
    expect(manifest.coverageTailMs).toBe(5000 - 4500); // 500
  ...
  test("pair validator fails if missing open anchor", async () => {
    ...
    expect(manifest.validationErrors).toContain("Replay log is missing explicit Chainlink open anchor (market_price event).");
    expect(manifest.pairValidity).toBe("invalid");
  });

  test("pair validator fails if missing close truth", async () => {
    const replayLog = path.join(tmpDir, "missing-close.log");
    ...
    expect(manifest.validationErrors).toContain("Replay log is missing explicit Chainlink close/settlement truth.");
    expect(manifest.pairValidity).toBe("invalid");
  });

  test("pair validator passes a minimal synthetic complete pair with both open and close truth", async () => {
    const replayLog = path.join(tmpDir, "minimal-synthetic.log");
