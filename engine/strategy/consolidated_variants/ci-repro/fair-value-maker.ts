...
export const fairValueMaker: Strategy = async (ctx) => {
  ...
  const evaluateQuotes = () => {
    ...
    let remainingExposureBudget = Math.max(0, ctx.maxOpenExposureUsd - ctx.openExposureUsd);
    ...
    if (bidPriceUp !== null && bidPriceUp > 0.01 && bidPriceUp < 0.99 && evUp.edge >= config.minEdge && allowUpFlow) {
      if (!existingUp || Math.abs(existingUp.price - bidPriceUp) > (TOLERANCE + EPSILON)) {
        ...
        if (notional > remainingExposureBudget) {
          sharesToBuy = Math.floor(remainingExposureBudget / bidPriceUp);
          if (sharesToBuy >= 1) {
             ctx.log(`[fair-value] Clamping UP shares to ${sharesToBuy.toFixed(2)} due to remaining exposure budget ($${remainingExposureBudget.toFixed(2)})`, "yellow");
          }
        }
        ...
        if (inventoryUp < config.maxInventory && sharesToBuy >= 1) {
          ...
          if (!isExposureBlocked(ctx, exposureBlockCooldowns, exposureKey, "UP", bidPriceUp, sharesToBuy)) {
            remainingExposureBudget = Math.max(0, remainingExposureBudget - (bidPriceUp * sharesToBuy));
            ordersToPost.push({
              req: {
                tokenId: upTokenId,
                action: "buy" as const,
        ...
        } else if (sharesToBuy < 1 && remainingExposureBudget < 1) {
           if (ctx.clock.nowMs() % 10000 === 0) {
             ctx.log(`[fair-value] Suppressing UP quote: insufficient exposure budget ($${remainingExposureBudget.toFixed(2)})`, "dim");
           }
        }
      }
    } else if (existingUp) {
    ...
    if (bidPriceDown !== null && bidPriceDown > 0.01 && bidPriceDown < 0.99 && evDown.edge >= config.minEdge && allowDownFlow) {
      if (!existingDown || Math.abs(existingDown.price - bidPriceDown) > (TOLERANCE + EPSILON)) {
        ...
        if (notional > remainingExposureBudget) {
          sharesToBuy = Math.floor(remainingExposureBudget / bidPriceDown);
          if (sharesToBuy >= 1) {
             ctx.log(`[fair-value] Clamping DOWN shares to ${sharesToBuy.toFixed(2)} due to remaining exposure budget ($${remainingExposureBudget.toFixed(2)})`, "yellow");
          }
        }
        ...
        if (inventoryUp > -config.maxInventory && sharesToBuy >= 1) {
          ...
          if (!isExposureBlocked(ctx, exposureBlockCooldowns, exposureKey, "DOWN", bidPriceDown, sharesToBuy)) {
            remainingExposureBudget = Math.max(0, remainingExposureBudget - (bidPriceDown * sharesToBuy));
            ordersToPost.push({
              req: {
                tokenId: downTokenId,
                action: "buy" as const,
           ...
           if (ctx.clock.nowMs() % 10000 === 0) {
             ctx.log(`[fair-value] Suppressing DOWN quote: insufficient exposure budget ($${remainingExposureBudget.toFixed(2)})`, "dim");
           }
        }
      }
