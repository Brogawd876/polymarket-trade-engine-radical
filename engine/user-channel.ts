...
export class SimUserChannel extends UserChannelBase {
  ...
  override trackOrder(orderId: string, request: OrderRequest): void {
    ...
    // Enforce realistic queue entry
    const book = this._getBook(request.req.tokenId);
    let queueAhead = 0;
    if (book && "bids" in book) {
      const fb = book as FillModelBook;
      const levels = request.req.action === "buy" ? fb.bids : fb.asks;
      const atLevel = levels.find(([price]) => Math.abs(price - request.req.price) < 1e-9);
      queueAhead = atLevel ? atLevel[1] : 0;
    }

    this._simState.set(orderId, {
      queueAhead,
      placementTsMs: this._clock.nowMs(),
  ...
  private _checkFill(
    ...
    const state = this._simState.get(orderId);
    ...
    if (isConservative && "bids" in bookData && "asks" in bookData) {
      ...
      // The queue ahead of us can never be larger than the total current depth at our price level.
      // If the real market depth shrinks (via trades or cancels), our queue position advances.
      // If real depth grows, those orders join behind us, so our queue position doesn't increase.
      const levels = order.action === "buy" ? (bookData as FillModelBook).bids : (bookData as FillModelBook).asks;
      const atLevel = levels.find(([price]) => Math.abs(price - order.price) < 1e-9);
      const currentDepth = atLevel ? atLevel[1] : 0;

      // Track exact trades to deplete queue precisely BEFORE bounding by current depth
      if (bookData.lastTradePrice !== undefined && bookData.lastTradePrice !== null) {
         if (Math.abs(bookData.lastTradePrice - order.price) < 1e-9 && bookData.lastTradeSize && bookData.lastTradeTs) {
            if (bookData.lastTradeTs > state.lastProcessedTradeTs) {
               state.queueAhead = Math.max(0, state.queueAhead - bookData.lastTradeSize);
               state.lastProcessedTradeTs = bookData.lastTradeTs;
            }
         }
      }

      state.queueAhead = Math.min(state.queueAhead, currentDepth);
      ...
      const result = model.evaluate(
        {
          ...
          queuePosition: state.queueAhead,
        },
